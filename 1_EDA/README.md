...
A SQL project analyzing the data engineer job market using real world job posting data. It demostrates my ability to **write production quality analytical SQL, design efficient queries and turn business questions into data driven insights**. 
...
2. [`02_top_paying_skilss.sql`](./02_top_paying_skills.sql) - salary analysis with aggregations
3. [`03_optimal_slills.sql`](./03_optimal_skills.sql) - combined demand/salary optimization query
...
This project analyzes a **data warehouse** built using a star schema design. The warehouse structure consists off: 
...
- **Data Model:** Star schema with fact + dimesion + bridge tables
...
2. **[`02_top_paying_skilss.sql`](./02_top_paying_skills.sql)** - Analyzes the 25 highest paying skills with salary and demand metrics
3. **[`03_optimal_slills.sql`](./03_optimal_skills.sql)** - Calculates an optimal score using natural log of demand combined with median salary to identify the most valuable skills to learn 
...
- **Complex Joins**: Multi-table `INNER JOIN` operations across `job_postings_fact`, `skills_job_dim`, `skills_dim`
...
- **Sorting & Limiting**: `ORDER BY` wth `DESC` and `LIMIT` for top-N analysis

### Data Analysis Techniques

- **Grouping**: `GROUP BY` for categorial analysis by skill
- **Conditional Logic**: `CASE WHEN` statements for derived metrics 
- **Mathematical Fuctions**: `LN()` for natural logarithm transformation to normalize demand metrics
